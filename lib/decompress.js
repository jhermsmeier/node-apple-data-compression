...
// The value of 128 was choses as it's the next power of 2 from 67
...
function decodeData( buffer, position ) {
...
function decodeRunlength( buffer, position ) {
...
function decompress( buffer ) {
  ...
  while( position < buffer.length ) {
    ...
    switch( chunkType ) {
      case ADC.CHUNK_DATA:
        chunks.push( decodeData( buffer, position ) )
        position = position + ADC.getChunkLength( buffer[ position ] ) + 1
        break
      case ADC.CHUNK_THREE:
        chunks.push( decodeRunlength( buffer, position ) )
        position = position + 3
        break
      case ADC.CHUNK_TWO:
        chunks.push( decodeRunlength( buffer, position ) )
